...
export class PopupService {
    ...
    private getPopupParent(): HTMLElement {
        return this.dropDownCore.getRootGui();
    }
    ...
    public positionPopupForMenu(params: {eventSource: any, ePopup: HTMLElement}) {
        ...
        let x: number;
        // if (this.dropdownOptionsWrapper.isEnableRtl()) {
    ...
    private callPostProcessPopup(ePopup: HTMLElement, eventSource: HTMLElement, mouseEvent: MouseEvent|Touch, type:string): void {
        let callback = this.dropdownOptionsWrapper.getPostProcessPopupFunc();
        if (callback) {
    ...
    public addAsModalPopup(eChild: any, closeOnEsc: boolean, closedCallback?: ()=>void): (event?: any)=>void {
        let eBody = this.dropdownOptionsWrapper.getDocument();
        if (!eBody) {
